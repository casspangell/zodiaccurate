...
function getChatGPTResponse(instructions, jsonData, uuid, emailAddress) {
  var apiKey = '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';
  ...
  var jsonResponse = UrlFetchApp.fetch(url, options);
    console.log("Z-GET CHATGPT RESPONSE: "+jsonResponse);
  ...
  if (isValidJson(jsonResponse)) {
    ...
    if (parsedResponse.choices && parsedResponse.choices.length > 0) {
      ...
      console.log("Z-RESPONSE DATA: "+JSON.stringify(parsedResponse));
...
function parseJSONResponse(parsedResponse){
  ...
  if (parsedResponse.usage) {
      console.log("Z-PROMPT TOKENS: " + parsedResponse.usage.prompt_tokens);
      console.log("Z-COMPLETION TOKENS: " + parsedResponse.usage.completion_tokens);
      console.log("Z-TOTAL TOKENS: " + parsedResponse.usage.total_tokens);
  } else {
      console.log("Z-Usage data not available in the response.");
  }
